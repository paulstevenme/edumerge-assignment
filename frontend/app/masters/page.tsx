...
export default function MastersPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setError('');
    try {
      await apiFetch('/masters', { method: 'POST', body: JSON.stringify(form) });
        ...
        <div className="card">
          <form className="grid grid-2" onSubmit={handleSubmit}>
            ...
            <div className="field">
              ...
              <select value={form.admissionMode} onChange={(e) => setForm({ ...form, admissionMode: e.target.value })}>
                <option>Government</option><option>Management</option>
              </select>
            </div>
            {message ? <div className="success">{message}</div> : null}
            {error ? <div className="error">{error}</div> : null}
            <div><button className="btn" type="submit">Save Master</button></div>
          </form>
        </div>
        <div className="card">
          <h3>Saved Master Records</h3>
          <table className="table">
            <thead><tr><th>Institution</th><th>Campus</th><th>Department</th><th>Year</th><th>Mode</th></tr></thead>
            <tbody>
              {masters.map((item) => (
                <tr key={item._id}>
